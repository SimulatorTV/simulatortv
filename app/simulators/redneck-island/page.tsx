...
export default function RedneckIslandSimulator() {
  ...
  function visibleVoteCountTeams() {
    const counts = currentVoteCounts();

    return rankedTeams
      .filter((team) => (counts[team.id] || 0) > 0)
      .sort((a, b) => {
        const voteDiff = (counts[b.id] || 0) - (counts[a.id] || 0);
        if (voteDiff !== 0) return voteDiff;
        return (teamPlacement(a.id) || 999) - (teamPlacement(b.id) || 999);
      });
  }
